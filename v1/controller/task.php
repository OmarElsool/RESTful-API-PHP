...
if(array_key_exists("taskid",$_GET)){
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        try{
            ...
            $query = $readDB->prepare("SELECT id, title, description, DATE_FORMAT(deadline, '%d/%m/%Y %H:%i') as deadline, completed from tbltasks where id = :taskid"); // : before taskid for injection and security
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        try{
            ...
            $query = $readDB->prepare("SELECT id, title, description, DATE_FORMAT(deadline, '%d/%m/%Y %H:%i') as deadline, completed from tbltasks where completed = :completed"); // : before taskid for injection and security
    ...
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
        ...
        try{
            ...
            $query = $readDB->prepare("SELECT id, title, description, DATE_FORMAT(deadline, '%d/%m/%Y %H:%i') as deadline, completed from tbltasks LIMIT :pglimit offset :offset"); // offset which row start with
...
}elseif(empty($_GET)){ // if GET is empty mean we get all tasks not specific id or complete
    // /tasks
    if($_SERVER['REQUEST_METHOD'] === 'GET'){
